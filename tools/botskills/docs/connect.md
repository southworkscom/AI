...
```bash
botskills connect --localManifest "./skills/customSkill/customSkillManifest.json" --appSettingsFile "./appsettings.json" --cs --verbose
```
...
```bash
botskills connect --remoteManifest "http://<YOUR_SKILL_MANIFEST>.azurewebsites.net/api/skill/manifest" --appSettingsFile "./appsettings.json" --cs --verbose
```
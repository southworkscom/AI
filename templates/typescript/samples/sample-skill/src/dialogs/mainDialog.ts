...
export class MainDialog extends ComponentDialog {
            ...
            if (generalResult.intents[intent].score > 0.5) {
                switch(intent) {
                    case 'Cancel': { 
                        ...
                        await innerDc.context.sendActivity(this.templateManager.generateActivityForLocale('CancelledMessage', innerDc.context.activity.locale as string));
                    ...
                    case 'Help': {
                        ...
                        await innerDc.context.sendActivity(this.templateManager.generateActivityForLocale('HelpCard', innerDc.context.activity.locale as string));
                    ...
                    case 'Logout': {
                        ...
                        await innerDc.context.sendActivity(this.templateManager.generateActivityForLocale('LogoutMessage', innerDc.context.activity.locale as string));
                        await innerDc.cancelAllDialogs();
                        await innerDc.beginDialog(this.initialDialogId);
                        interrupted = true;
                        break;
    ...
    private async introStep(stepContext: WaterfallStepContext): Promise<DialogTurnResult> {
        if (TurnContextEx.isSkill(stepContext.context)) {
            ...
            const promptOptions: PromptOptions = {
                prompt: (stepContext.options as Activity).type !== undefined ? stepContext.options : this.templateManager.generateActivityForLocale('FirstPromptMessage', stepContext.context.activity.locale as string)
            };
            ...
            if (luisService !== undefined){
                ...
                switch(intent) {
                    ...
                    default: {
                        ...
                        await stepContext.context.sendActivity(this.templateManager.generateActivityForLocale('UnsupportedMessage', stepContext.context.activity.locale as string));
                        return await stepContext.next();
                    }
                }  
            } else {
    ...
    private async finalStep(stepContext: WaterfallStepContext): Promise<DialogTurnResult> {
        if (TurnContextEx.isSkill(stepContext.context)) {
            ...
            return await stepContext.replaceDialog(this.id, this.templateManager.generateActivityForLocale('CompletedMessage', stepContext.context.activity.locale as string));
        }
    }
